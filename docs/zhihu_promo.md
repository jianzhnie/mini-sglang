# 5,200 行 Python 手搓一个 SGLang：把 LLM 推理引擎扒光了给你看
...
所以我写了 **Mini-SGLang**：用 **~3,000 行纯 Python** 完整复刻 SGLang 的核心机制。不是玩具 demo，是一个真的能跑模型、能起 OpenAI 兼容服务、能上 NPU 的推理引擎——只是每一行都为你阅读而写。
...
模型支持：OPT / Qwen2 / Qwen3 / Qwen3-MoE / Llama / Mistral。
...
**2. 模型继承体系消掉了 90% 的重复代码。** Qwen2/Qwen3/Llama/Mistral 共用一套基类：
...
每个模型文件只需写自己独特的 Attention 类，**约 50 行**。

**3. 不是"能跑就行"，是真的对。** 104 个单元测试全绿，RadixCache 前缀共享与全量 prefill 的 logits 级数值一致性、double-free 防护、CUDA Graph 的静态 buffer 正确性……这些生产级引擎里最容易出错的点，都有回归测试盯着。
...
通过 `torch_npu` 做了完整的设备无关适配，Ascend 910 上 6 个模型全部实测通过：

| 模型 | Prefill | Decode | 批量(3x)吞吐 |
|------|---------|--------|-------------|
| Qwen2.5-0.5B | 395.5 tok/s | 24.2 tok/s | 72.6 tok/s |
| Qwen2.5-1.5B | 336.5 tok/s | 20.8 tok/s | 60.8 tok/s |
