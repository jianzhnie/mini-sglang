...
class BatchContext:
    ...
    def __init__(self, max_running_req: int, max_seq_len: int, page_size: int, device: torch.device):
    ...
    def prepare(self, batch: Batch) -> None:
            ...
            if req.cache_handle:
                for pos in range(req.cached_len, len(req.input_ids)):
                    ...
                    if page_idx < len(req.cache_handle.page_ids):
                        loc = req.cache_handle.page_ids[page_idx] * self.page_size + offset
        ...
        batch.input_ids = torch.tensor(all_input_ids, dtype=torch.long, device=self.device)
        batch.positions = torch.tensor(all_positions, dtype=torch.long, device=self.device)
        ...
        num_reqs = len(reqs)
        table = torch.full((num_reqs, self.max_seq_len), -1, dtype=torch.int32, device=self.device)
        for i, req in enumerate(reqs):
            if req.cache_handle:
                for pos in range(len(req.input_ids)):
                    ...
                    if page_idx < len(req.cache_handle.page_ids):
                        offset = pos % self.page_size
                        table[i, pos] = req.cache_handle.page_ids[page_idx] * self.page_size + offset
